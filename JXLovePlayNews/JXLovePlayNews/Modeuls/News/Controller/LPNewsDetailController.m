...
static NSString * const headerId = @"LPNewsTitleSectionView";
static NSString * const footerId = @"LPNewsCommentFooterView";
static CGFloat const kHeaderViewHeight = 164;
...
- (void)loadData
{
    ...
    LPWeakSelf(self);
    [LPNewsViewModel requestNewsDetailWithNewsId:self.newsId completion:^(LPNewsDetailModel * detailModel) {
        LPStrongSelf(self);
        
        if (detailModel) {
            [self replaceDetailImageWithArticle:detailModel.article];
            dispatch_async(dispatch_get_main_queue(), ^{
                _newsDetail = detailModel;
                _favors = detailModel.article.relative_sys;
...
- (ASCellNodeBlock)tableView:(ASTableView *)tableView nodeBlockForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        ...
        ASCellNode *(^commentCellNodeBlock)() = ^ASCellNode *() {
            ...
            NSDictionary *dict = _newsDetail.tie.comments;
            
            LPNewsCommentCellNode *cellNode = [[LPNewsCommentCellNode alloc] initWithCommentItems:dict floors:floors];
            return cellNode;
        };
